...
- 거시경제에이전트는 30분에 한번씩 gemini search grounded api 를 사용해서 뉴스 감성분석 진행, 거시경제 보고서 작성(보고서 파일은 백업없이 갱신, 요청이 들어오면 파일의 내용을 읽어서 반환)
- 거래종목 선택 에이전트는 거시경제 보고서 결과로부터 주목할 분야를 선정하고, 해당 분야의 키워드 크롤링을 통한 뉴스 감성 분석을 통해 거래할 후보 종목들을 선정해서 파일로 정리(파일은 백업없이 갱신, 요청이 들어오면 파일의 내용을 읽어서 반환)
- 포트폴리오 에이전트는 (거래량을 보고있다가 거래량이 많을 때에는 5분, 거래량이 적을 때에는 10분에 한번씩) 거래종목 선택 에이전트와 거시경제에이전트에게 각각 POST로 거시경제 분위기와 종목을 받는다.
- 포트폴리오 에이전트는 받은 종목들의 ticker를 기술적 분석 에이전트와 기업분석 에이전트에 POST 요청으로 전달해서 각 종목 별 기술분석과 기업분석을 진행하고 결과를 반환받음
- 거시경제, 기술분석 및 기업분석을 통해 거래 여부를 결정하고 운용가능 자금을 분산해서 구매 (보유 금액이 적으면 구매하지 않음음)
...
### 3.6 포트폴리오 관리 에이전트 (Master Agent)
